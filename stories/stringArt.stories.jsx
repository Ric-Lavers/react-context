...
class FullScreenSVG extends Component {
  ...
  handleClick = (event) => {
    ...
    const {array:intervalX, interval:lenIntervalX} = findIntervals(beforeLast[0], last[0])
    const {array:intervalY, interval:lenIntervalY} = findIntervals(beforeLast[1], last[1])
    const {array:intervalCurrentX, interval:lenCurrentIntervalX} = findIntervals(last[0], current[0])
    const {array:intervalCurrentY, interval:lenCurrentIntervalY} = findIntervals(last[1], current[1])
    ...
    for(let i=0;i<intervalX.length-1;i++){
      art.push([ intervalX[i], intervalY[i], intervalCurrentX[i], intervalCurrentY[i] , intervalCurrentX[i]-lenCurrentIntervalX, intervalCurrentY[i]-lenCurrentIntervalY ])
    }
    let pattern = art
